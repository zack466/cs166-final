...
# Ns = 5:7
# combs = Iterators.product(keys(images), Ns)
# ggs = Dict(combs, map(combs) do (name, N)
#                gmm_3d_spatial(images[name], N)
#            end)
#            end)
#
...
#     imwrite("$(OUTPUT_DIR)/$(base2)-to-$(base1)-spatial-$(N).png", comp)
# end
#
...
#
# @Threads.threads for (name, N) in combs
#     ggg[(name, N)] = gmm_3d_spatial(greyscale(images[name]), N)
# end
#
# exps = collect(Iterators.product(keys(images), keys(images), Ns))
#
...
#
#
# # Colorization - manual color selection
#
