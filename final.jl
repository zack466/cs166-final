...
using Images, FileIO, ImageSegmentation, Statistics, Profile

# ╔═╡ 0d161a18-0a4e-47bb-a190-dca4b7cbc50f
begin
	img = Lab.(load("./images/man-lowres.png"))
	pixels = channelview(img)[:, :, :]
	c, h, w = size(pixels)
end
...
function estep(pixels, means, stds, N)
	probs = zeros(N, c, h, w)
	for i in 1:N
		probs[i, :, :, :] = exp.(-((pixels .- means[:, i]).^2 ./ (2 .* stds[:, i].^2))) .+ 1e-9
	end
...
function mstep(pixels, probs, N)
	...
	stds = zeros(3, N)
	for i in 1:N
		Z = sum(probs[i, :, :, :], dims=(2, 3))
		means[:, i] = sum(probs[i, :, :, :] .* pixels, dims=(2, 3)) ./ Z
		stds[:, i] = (sum(probs[i, :, :, :] .* (pixels .- means[:, i]).^2, dims=(2, 3)) ./ Z) .^ 0.5
	end
	return means, stds
end

# ╔═╡ f28f2393-c45a-4410-b2f7-4b39891277b7
function gmm_3d(pixels, N)
	c, h, w = size(pixels)
	K = kmeans(stack(eachpixel(pixels)), N)  # initialize with k-means clusters
	means = K.centers  # 3 x N
	stds = hcat([
	stdm(
		getindex(eachpixel(pixels), findall(==(i), K.assignments)), K.centers[:, i])
		for i in 1:N
	]...)
	# means = stack(rand(eachpixel(pixels), N))
	# stds = ones(3, N) .* 5
	...
	while t < 50
		...
		diff = abs(sum(new_means .- means) / sum(means))
		probs = new_probs
		means = new_means
		stds = new_stds
		if diff < 0.0005
...
begin
	cluster = kmeans(stack(eachpixel(pixels)), N)
	map(reshape(cluster.assignments, size(img))) do i
		l, a, b = cluster.centers[:, i]
		Lab(l, a, b)
	end
end

# ╔═╡ 50a170ca-1c96-43e1-9fe6-c4a8432e7f9a
function gmm_3d_spatial(pixels, N)
	c, h, w = size(pixels)
	probs = zeros(N, c, h, w)
	while true
		estep()
...
manifest_format = "2.0"
project_hash = "f1cedff2091febe983602083e70b07da428e3d19"
